...
export default function CashHandlingTwoDollarScreen() {
  ...
  const [totalCash, setTotalCash] = useState(210);
  const [cashRequest, setCashRequest] = useState(100);
  ...
  const generateNewQuestion = () => {
    ...
    // Calculate total cash value (payout × $2)
    const total = payout * 2;
    setTotalCash(total);

    // Determine valid cash requests (multiples of 50, must be < total)
    const validCashOptions: number[] = [];
    for (let cash = 50; cash < total; cash += 50) {
    ...
    // Calculate remaining chips: (total - cash) / 2
    const calculatedChips = (total - selectedCash) / 2;
  ...
  const getExplanation = (): string => {
    return `Payout: ${chips} × 35 = ${payoutChips} chips\nTotal cash: ${payoutChips} × $2 = $${totalCash}\nPayout breakdown: ${remainingChips} chips ($${remainingChips * 2}) + $${cashRequest} cash = $${totalCash}`;
  };
    ...
    <View style={styles.container}>
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent}>
        ...
        <HintSection isOpen={showHint} onToggle={() => setShowHint(!showHint)}>
          • Chip denomination: <Text style={styles.highlightNumber}>$2</Text>{'\n'}
          ...
          • Total cash value: <Text style={styles.highlightNumber}>{payoutChips} × $2 = ${totalCash}</Text>{'\n'}
          {'\n'}
          <Text style={styles.hintTitle}>Cash Handling:{'\n'}</Text>
          • Total must equal: <Text style={styles.highlightNumber}>${totalCash}</Text>{'\n'}
          • Formula: (Chips × $2) + Cash = Total{'\n'}
          • Therefore: {remainingChips} chips (${remainingChips * 2}) + ${cashRequest} cash = ${totalCash}{'\n'}
        </HintSection>
