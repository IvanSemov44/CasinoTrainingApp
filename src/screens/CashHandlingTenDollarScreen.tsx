...
export default function CashHandlingTenDollarScreen() {
    ...
    const [totalCash, setTotalCash] = useState(1050);
    const [cashRequest, setCashRequest] = useState(400);
    const [remainingChips, setRemainingChips] = useState(65);
    ...
    const generateNewQuestion = () => {
        ...
        // Calculate total cash value (payout × $10)
        const total = payout * 10;
        setTotalCash(total);

        // Determine valid cash requests (multiples of 100, must be < total)
        const validCashOptions: number[] = [];
        for (let cash = 100; cash < total; cash += 100) {
        ...
        // Calculate remaining chips: (total - cash) / 10
        const calculatedChips = (total - selectedCash) / 10;
    ...
    const getExplanation = (): string => {
        return `Payout: ${chips} × 35 = ${payoutChips} chips\nTotal cash: ${payoutChips} × $10 = $${totalCash}\nPayout breakdown: ${remainingChips} chips ($${remainingChips * 10}) + $${cashRequest} cash = $${totalCash}`;
    };
        ...
        <View style={styles.container}>
            <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent}>
                ...
                <HintSection isOpen={showHint} onToggle={() => setShowHint(!showHint)}>
                    • Chip denomination: <Text style={styles.highlightNumber}>$10</Text>{'\n'}
                    ...
                    • Total cash value: <Text style={styles.highlightNumber}>{payoutChips} × $10 = ${totalCash}</Text>{'\n'}
                    {'\n'}
                    <Text style={styles.hintTitle}>Cash Handling:{'\n'}</Text>
                    • Total must equal: <Text style={styles.highlightNumber}>${totalCash}</Text>{'\n'}
                    • Formula: (Chips × $10) + Cash = Total{'\n'}
                    • Therefore: {remainingChips} chips (${remainingChips * 10}) + ${cashRequest} cash = ${totalCash}{'\n'}
                </HintSection>
