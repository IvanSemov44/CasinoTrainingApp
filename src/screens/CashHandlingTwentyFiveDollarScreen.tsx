...
export default function CashHandlingTwentyFiveDollarScreen() {
  ...
  const [totalCash, setTotalCash] = useState(2625);
  const [cashRequest, setCashRequest] = useState(1000);
  ...
  const generateNewQuestion = () => {
    ...
    // Calculate total cash value (payout × $25)
    const total = payout * 25;
    setTotalCash(total);
    ...
    // Calculate remaining chips: (total - cash) / 25
    const calculatedChips = (total - selectedCash) / 25;
  ...
  const getExplanation = (): string => {
    return `Payout: ${chips} × 35 = ${payoutChips} chips\nTotal cash: ${payoutChips} × $25 = $${totalCash}\nPayout breakdown: ${remainingChips} chips ($${remainingChips * 25}) + $${cashRequest} cash = $${totalCash}`;
  };
    ...
    <View style={styles.container}>
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent}>
        ...
        <HintSection isOpen={showHint} onToggle={() => setShowHint(!showHint)}>
          • Chip denomination: <Text style={styles.highlightNumber}>$25</Text>{'\n'}
          ...
          • Total cash value: <Text style={styles.highlightNumber}>{payoutChips} × $25 = ${totalCash}</Text>{'\n'}
          {'\n'}
          <Text style={styles.hintTitle}>Cash Handling:{'\n'}</Text>
          • Total must equal: <Text style={styles.highlightNumber}>${totalCash}</Text>{'\n'}
          • Formula: (Chips × $25) + Cash = Total{'\n'}
          • Therefore: {remainingChips} chips (${remainingChips * 25}) + ${cashRequest} cash = ${totalCash}{'\n'}
        </HintSection>
