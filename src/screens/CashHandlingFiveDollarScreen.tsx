...
export default function CashHandlingFiveDollarScreen() {
  ...
  const [totalCash, setTotalCash] = useState(525);
  const [cashRequest, setCashRequest] = useState(200);
  ...
  const generateNewQuestion = () => {
    ...
    // Calculate total cash value (payout × $5)
    const total = payout * 5;
    setTotalCash(total);
    ...
    // Calculate remaining chips: (total - cash) / 5
    const calculatedChips = (total - selectedCash) / 5;
  ...
  const getExplanation = (): string => {
    return `Payout: ${chips} × 35 = ${payoutChips} chips\nTotal cash: ${payoutChips} × $5 = $${totalCash}\nPayout breakdown: ${remainingChips} chips ($${remainingChips * 5}) + $${cashRequest} cash = $${totalCash}`;
  };
    ...
    <View style={styles.container}>
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent}>
        ...
        <HintSection isOpen={showHint} onToggle={() => setShowHint(!showHint)}>
          • Chip denomination: <Text style={styles.highlightNumber}>$5</Text>{'\n'}
          ...
          • Total cash value: <Text style={styles.highlightNumber}>{payoutChips} × $5 = ${totalCash}</Text>{'\n'}
          {'\n'}
          <Text style={styles.hintTitle}>Cash Handling:{'\n'}</Text>
          • Total must equal: <Text style={styles.highlightNumber}>${totalCash}</Text>{'\n'}
          • Formula: (Chips × $5) + Cash = Total{'\n'}
          • Therefore: {remainingChips} chips (${remainingChips * 5}) + ${cashRequest} cash = ${totalCash}{'\n'}
        </HintSection>
