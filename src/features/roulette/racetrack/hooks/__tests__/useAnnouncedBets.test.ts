...
import { BetType, type PlacedBet } from '@app-types/roulette.types';
...
describe('useAnnouncedBets', () => {
  ...
  describe('handleSectionPress', () => {
    ...
    it('should not place bets for invalid section', () => {
      ...
      act(() => {
        result.current.handleSectionPress(
          'invalid' as unknown as Parameters<typeof result.current.handleSectionPress>[0]
        );
      });
