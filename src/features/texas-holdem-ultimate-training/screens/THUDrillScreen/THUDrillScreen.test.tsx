...
describe('THUDrillScreen', () => {
  ...
  describe('Rendering', () => {
    it('renders without crashing', () => {
      const { toJSON } = renderWithTheme(
        <THUDrillScreen navigation={mockNavigation} route={mockRoute} />
      );
  ...
  describe('Props and Params', () => {
    it('uses route params correctly', () => {
      ...
      const { toJSON } = renderWithTheme(
        <THUDrillScreen navigation={mockNavigation} route={customRoute} />
      );
      expect(toJSON()).toBeTruthy();
    });
  });
  ...
  describe('Different Drill Types', () => {
    it('handles multiple drill types', () => {
      ...
      drillTypes.forEach(drillType => {
        ...
        const { toJSON } = renderWithTheme(
          <THUDrillScreen navigation={mockNavigation} route={route} />
        );
        expect(toJSON()).toBeTruthy();
      });
    });
