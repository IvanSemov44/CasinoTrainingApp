...
function getColumn(n: number): 1 | 2 | 3 {
  return n % 3 === 0 ? 3 : ((n % 3) as 1 | 2);
}
function getDozen(n: number): 1 | 2 | 3 {
...
function isLow(n: number): boolean {
  return n >= 1 && n <= 18;
}
...
function getWinningBetsForNumber(n: number): OutsideBetPlaced[] {
  ...
  const bets: OutsideBetPlaced[] = [
    ...
    {
      name: `Dozen ${getDozen(n)} (${getDozen(n) === 1 ? '1–12' : getDozen(n) === 2 ? '13–24' : '25–36'})`,
      multiplier: 2,
      amount: 0,
    },
  ];
...
export function generateRKScenario(drillType: RKDrillType): RKScenario {
  switch (drillType) {
    case 'outside-bet-payout':
      return generateOutsideBetPayout();
    case 'dozen-vs-column':
      return generateDozenVsColumn();
    case 'zero-rule':
      return generateZeroRule();
    case 'outside-bet-recognition':
      return generateOutsideBetRecognition();
    case 'announced-chip-count':
      return generateAnnouncedChipCount();
    case 'announced-numbers':
      return generateAnnouncedNumbers();
    case 'announced-composition':
      return generateAnnouncedComposition();
    case 'mixed-outside-payout':
      return generateMixedOutsidePayout();
    case 'announced-inside-mixed':
      return generateAnnouncedInsideMixed();
    case 'bet-limits':
      return generateBetLimits();
  }
}
