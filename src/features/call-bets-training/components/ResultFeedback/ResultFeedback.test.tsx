...
describe('ResultFeedback', () => {
  ...
  describe('edge cases', () => {
    ...
    it('should handle result with multiple extra bets', () => {
      const multipleExtraResult: ValidationResult = {
        isCorrect: false,
        correctBets: [],
        userBets: [],
        missingBets: [],
        extraBets: [
          { type: 'split', numbers: [1, 2] },
          { type: 'street', numbers: [4, 5, 6] },
        ],
        score: 0,
      };
      const { getByText } = render(
        <ResultFeedback result={multipleExtraResult} onNext={mockOnNext} onClear={mockOnClear} />
      );
      expect(getByText(/split: 1, 2/)).toBeTruthy();
      expect(getByText(/street: 4, 5, 6/)).toBeTruthy();
    });

    it('should handle result with both missing and extra bets', () => {
      const mixedResult: ValidationResult = {
        isCorrect: false,
        correctBets: [],
        userBets: [],
        missingBets: [{ type: 'split', numbers: [7, 10] }],
        extraBets: [{ type: 'corner', numbers: [1, 2, 4, 5] }],
        score: 25,
      };
      const { getByText } = render(
        <ResultFeedback result={mixedResult} onNext={mockOnNext} onClear={mockOnClear} />
      );
      expect(getByText('Missing Positions:')).toBeTruthy();
      expect(getByText('Extra/Wrong Positions:')).toBeTruthy();
    });

    it('should handle high score but with errors', () => {
      const highScoreResult: ValidationResult = {
        isCorrect: false,
        ...
        missingBets: [],
        extraBets: [{ type: 'split', numbers: [10, 13] }],
        score: 75,
      };
      const { getByText } = render(
        <ResultFeedback result={highScoreResult} onNext={mockOnNext} onClear={mockOnClear} />
      );
      expect(getByText('Score: 75%')).toBeTruthy();
      expect(getByText(/Not Quite/)).toBeTruthy();
    });
  });
});
