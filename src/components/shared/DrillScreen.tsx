...
interface DrillScreenProps<TScenario extends BaseDrillScenario = BaseDrillScenario> {
  scenarioGenerator: (drillType: any) => TScenario;
  drillType: any;
  betChipLabel?: (drillType: any) => string;
  dealerLabel?: (drillType: any) => string;
}

export default function DrillScreen<TScenario extends BaseDrillScenario = BaseDrillScenario>({
  scenarioGenerator,
  drillType,
  betChipLabel = () => 'BET',
  dealerLabel = () => 'DEALER',
}: DrillScreenProps<TScenario>) {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      {/* ── Cards ── */}
      {(scenario as any).playerCards && (scenario as any).playerCards.length > 0 && (
        <View style={styles.handBlock}>
          <Text style={styles.handLabel}>PLAYER</Text>
          <View style={styles.cardRow}>
            {(scenario as any).playerCards.map((card: any, i: number) => (
      ...
      {(scenario as any).dealerCards && (scenario as any).dealerCards.length > 0 && (
        <View style={styles.handBlock}>
          <Text style={styles.handLabel}>{dealerLabel(drillType)}</Text>
          <View style={styles.cardRow}>
            {(scenario as any).dealerCards.map((card: any, i: number) => (
              <PlayingCard key={i} card={card} size="lg" />
            ))}
          </View>
        </View>
      ...
      {(scenario as any).betAmount !== undefined && (
        <View style={styles.betChip}>
          <Text style={styles.betChipLabel}>{betChipLabel(drillType)}</Text>
          <Text style={styles.betChipValue}>€{(scenario as any).betAmount}</Text>
        </View>
      )}

      {/* ── Question ── */}
      <View style={styles.questionBox}>
        <Text style={styles.questionText}>{(scenario as any).question}</Text>
      </View>
        ...
        <>
          <View style={[styles.resultCard, isCorrect ? styles.resultCorrect : styles.resultIncorrect]}>
            ...
            <View style={styles.explanationBox}>
              <Text style={styles.explanationText}>{(scenario as any).explanation}</Text>
            </View>
